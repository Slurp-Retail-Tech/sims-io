...
--
-- Idempotency:
--   This is a FORWARD-ONLY migration. The ADD COLUMN / ADD CONSTRAINT statements
--   error if re-run on a DB that already has them (consistent with migrations
--   005-009). The two new tables use CREATE TABLE IF NOT EXISTS. The source
--   backfill UPDATE is naturally idempotent (re-running matches nothing new).
-- =============================================================================

-- 1. Lead assignment ---------------------------------------------------------
ALTER TABLE leads
  ADD COLUMN assigned_user_id BIGINT UNSIGNED DEFAULT NULL AFTER source,
