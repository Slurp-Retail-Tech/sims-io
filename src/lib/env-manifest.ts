...
export const ENV_MANIFEST: readonly EnvSpec[] = [
  ...
  {
    name: "COMMERCEPAY_BASE_URL",
    requirement: { kind: "optional" },
    why: "is optional — it defaults to the staging gateway, and must be set to the production host before real money moves",
    public: false,
    buildTime: false,
  },
  {
    name: "COMMERCEPAY_PASSWORD",
    requirement: {
      kind: "feature",
      feature: "CommercePay payment gateway",
      enabledWhen: ["COMMERCEPAY_TENANT_ID"],
    },
    why: "authenticates SIMS to the payment gateway; without it no payment link can be created",
    public: false,
    buildTime: false,
  },
  {
    name: "COMMERCEPAY_SECRET_KEY",
    requirement: {
      kind: "feature",
      feature: "CommercePay payment gateway",
      enabledWhen: ["COMMERCEPAY_TENANT_ID"],
    },
    why: "signs every gateway request and verifies every payment callback; without it a forged callback could not be told from a real one",
    public: false,
    buildTime: false,
  },
  {
    name: "COMMERCEPAY_TENANT_ID",
    requirement: { kind: "optional" },
    why: "is the merchant id that switches the payment gateway on; the other COMMERCEPAY_* values are required once it is set",
    public: false,
    buildTime: false,
  },
  {
    name: "COMMERCEPAY_USERNAME",
    requirement: {
      kind: "feature",
      feature: "CommercePay payment gateway",
      enabledWhen: ["COMMERCEPAY_TENANT_ID"],
  ...
  {
    name: "RENEWAL_SELLER_LINE_1",
    requirement: { kind: "optional" },
    why: "is optional — first address line printed under the seller name on renewal invoices and receipts",
    public: false,
    buildTime: false,
  },
  {
    name: "RENEWAL_SELLER_LINE_2",
    requirement: { kind: "optional" },
    why: "is optional — second address line on renewal documents",
    public: false,
    buildTime: false,
  },
  {
    name: "RENEWAL_SELLER_LINE_3",
    requirement: { kind: "optional" },
    why: "is optional — third address line on renewal documents, typically the registration number or contact email",
    public: false,
    buildTime: false,
  },
  {
    name: "RENEWAL_SELLER_NAME",
    requirement: { kind: "optional" },
    why: "is optional — the seller name printed on renewal documents; defaults to Slurp",
    public: false,
    buildTime: false,
  },
  {
  ...
  {
    name: "RESPONDIO_API_BASE_URL",
    requirement: { kind: "optional" },
    why: "is optional — it defaults to the documented Respond.io v2 endpoint",
    public: false,
    buildTime: false,
  },
  {
    name: "RESPONDIO_API_TOKEN",
    requirement: { kind: "optional" },
    why: "is optional — without it no renewal reminder or receipt can be sent, though invoices and payment links are still generated",
    public: false,
    buildTime: false,
  },
  {
    name: "RESPONDIO_EMAIL_CHANNEL_ID",
    requirement: {
      kind: "feature",
      feature: "renewal messaging over email",
      enabledWhen: ["RESPONDIO_API_TOKEN"],
    },
    why: "names the Respond.io channel renewal email is sent through; without it a send has no channel to route by",
    public: false,
    buildTime: false,
  },
  {
    name: "RESPONDIO_WHATSAPP_CHANNEL_ID",
    requirement: { kind: "optional" },
    why: "is optional — WhatsApp reminders are skipped until it is set and Meta has approved the message templates",
    public: false,
    buildTime: false,
  },
  {
